...
import { bestVerifiedGroceryPrice, groceryPriceObservations, observationsForProfile, priceObservationIsFresh } from "./grocery-pricing.js";
import { optimizeShoppingTrips } from "./shopping-trip-optimizer.js";
import { loadUserProfile, saveUserProfile } from "./user-profile.js";
import { supportedRetailers, exactStoreFor, normalizeDiscoveredStore, selectExactStore, storeDiscoverySummary } from "./store-discovery.js";
...
let shoppingOptimizationMode = null;
let meaningfulSavings = null;
...
shoppingOptimizationMode = userProfile.shopping?.optimizationMode || store.get("fk-shopping-optimization-mode", "fewest-trips");
meaningfulSavings = Number(userProfile.shopping?.meaningfulSavings ?? store.get("fk-shopping-meaningful-savings", 5));
...
function homeView() {
  ...
  <section class="dashboard section-shell"><div class="section-title"><div><p class="eyebrow">At a glance</p><h2>Your week in the kitchen</h2></div><p>${dinnerDays.length} dinners planned · ${week.days.length - dinnerDays.length} open evenings</p></div>
    <div class="dashboard-grid">
      <article class="panel plan-preview"><div class="panel-heading"><span class="icon-disc">${icons.leaf}</span><div><p class="tiny-label">Meals planned</p><h3>${dinnerDays.length} dinners, plenty of room</h3></div></div>${dinnerDays.map((day) => `<div class="mini-day"><strong>${day.day.slice(0, 3)}</strong><span>${escapeHtml(getRecipe(day.meals.dinner[0]).name)}</span></div>`).join("")}<a class="panel-link" href="#/week">Full weekly plan ${icons.arrow}</a></article>
...
function recipesView() {
  ...
  const visible = recipes.filter((recipe) => (recipeFilter === "all" || recipePreference(recipe.id) === recipeFilter) && (recipeCategoryFilter === "all" || categoryFor(recipe) === recipeCategoryFilter) && (recipeReadinessFilter === "all" || recipeStandardizationStatus(recipe) === recipeReadinessFilter));
  const filters = [
    ["all", "All"],
    ["favorite", "Favorites"],
  ...
  const readiness = [["all","All readiness"],["validated","Planning ready"],["ingredients","Ingredients standardized"],["source","Needs standardization"]];
  const readinessLabel = (recipe) => ({ validated:"Planning ready", portioning:"Portion ready", nutrition:"USDA matching", ingredients:"Ingredients standardized", source:"Needs standardization" }[recipeStandardizationStatus(recipe)] || "Needs standardization");
...
function shoppingView() {
  ...
  const localPriceObservations = observationsForProfile(userProfile, groceryPriceObservations);
  const pricedItems = items.map((item) => ({
    ...item,
    offers: localPriceObservations
      .filter((offer) => offer.ingredientKey === item.key && offer.verified === true && priceObservationIsFresh(offer))
      .map((offer) => ({ ...offer, storeId: String(offer.storeId), retailerKey: offer.store, totalPrice: Number(offer.salePrice ?? offer.price) }))
      .filter((offer) => Number.isFinite(offer.totalPrice)),
  }));
  const maxTrips = Number(userProfile.shopping?.maxTrips ?? 2);
  const optimized = optimizeShoppingTrips(pricedItems, { mode: shoppingOptimizationMode, meaningfulSavings, maxTrips });
  const modeLabels = { "fewest-trips": "Fewest trips", "balanced": "Balanced", "lowest-price": "Lowest price" };
  const assignedRetailerByKey = new Map((optimized.plan?.assignments || []).map((assignment) => [assignment.itemKey, assignment.offer.retailerKey]));
  const retailerName = (key) => stores.find((store) => store.id === key)?.label || key;
  const planStops = optimized.plan ? [...new Set(optimized.plan.assignments.map((assignment) => assignment.offer.retailerKey))] : [];
  const optimizerPanel = `<article class="detail-card shopping-optimizer"><p class="eyebrow">Preferred shopping plan</p><h2>${modeLabels[shoppingOptimizationMode]}</h2><div class="filter-row" role="group" aria-label="Shopping optimization mode">${Object.entries(modeLabels).map(([id,label]) => `<button class="chip ${shoppingOptimizationMode === id ? "active" : ""}" data-shopping-mode="${id}">${label}</button>`).join("")}</div><div class="progress-stats"><label><span>Maximum store trips</span><input type="number" min="1" max="5" step="1" value="${maxTrips}" data-max-shopping-trips></label><label><span>Nearby-store radius (miles)</span><input type="number" min="1" max="50" step="1" value="${Number(userProfile.shopping?.radiusMiles ?? 10)}" data-shopping-radius></label></div><label class="helper-text">Balanced mode adds another store only when it saves at least $<input type="number" min="0" step="1" value="${meaningfulSavings}" data-meaningful-savings>.</label>${optimized.plan ? `<p><strong>${optimized.plan.tripCount} stop${optimized.plan.tripCount === 1 ? "" : "s"} · ${optimized.plan.total.toFixed(2)} verified total</strong></p><p>${planStops.map(retailerName).join(" + ")}</p><p class="muted">Based only on fresh verified offers from your selected exact branches. ${optimized.unpriced.length ? `${optimized.unpriced.length} item${optimized.unpriced.length === 1 ? "" : "s"} still need fresh prices and are excluded from this total.` : "All listed items are priced."}</p>` : `<p class="muted">${optimized.reason || "A preferred route will appear after fresh verified prices are loaded for your selected exact branches."}</p>`}</article>`;
  const shopSection = (shop, title, subtitle) => { const visibleGroups = Object.fromEntries(Object.entries(groups[shop]).map(([category, categoryItems]) => [category, shoppingStoreFilter === "all" ? categoryItems : categoryItems.filter((item) => assignedRetailerByKey.get(item.key) === shoppingStoreFilter)]).filter(([, categoryItems]) => categoryItems.length)); const visibleCount = Object.values(visibleGroups).flat().length; return `<section class="shop-section"><div class="shop-heading"><div><p class="eyebrow">${subtitle}</p><h2>${title}</h2></div><span>${visibleCount} items</span></div>${Object.keys(visibleGroups).length ? Object.entries(visibleGroups).map(([category, categoryItems]) => `<article class="grocery-category"><h3>${category}</h3>${categoryItems.map((item) => { const id = `${shop}:${item.key}:${item.unitGroup ?? item.unit}`; return `<label class="check-row ${checked.includes(id) ? "checked" : ""}"><input type="checkbox" data-grocery="${id}" ${checked.includes(id) ? "checked" : ""}><span class="custom-check"></span><span>${item.item}${(() => { const price = bestVerifiedGroceryPrice(item.key, localPriceObservations); if (price.status !== "verified") return `<small class="muted">Price check pending</small>`; const best = [...price.winners].sort((a,b) => a.unitPrice - b.unitPrice)[0]; const displayPrice = Number(best.salePrice ?? best.price); return `<small>Best verified: ${retailerName(best.store)} · ${displayPrice.toFixed(2)}${best.loyaltyRequired ? " with loyalty" : ""}</small>`; })()}</span><strong>${item.displayAmount ?? `${formatAmount(item.amount)} ${item.unit}`}</strong></label>`; }).join("")}</article>`).join("") : `<div class="empty-state">${shoppingStoreFilter === "all" ? "Nothing on this list yet." : optimized.plan ? `No items assigned to ${retailerName(shoppingStoreFilter)} in the preferred plan.` : "Store assignments will appear after fresh local prices are available."}</div>`}</section>`; };
  return `<section class="section-shell page">${pageHeader("This week’s groceries", "Shopping", "One consolidated list, split around how you actually shop. Pantry staples you have are already filtered out.")}<div class="store-filter" role="group" aria-label="Filter shopping list by store">${stores.map((store) => `<button class="chip ${shoppingStoreFilter === store.id ? "active" : ""}" data-store-filter="${store.id}">${store.label}</button>`).join("")}</div><p class="helper-text store-filter-note">${shoppingStoreFilter === "all" ? "Showing the complete list. Each ingredient shows the lowest verified store listing when current comparable price data is available." : `Showing items assigned to ${stores.find((store) => store.id === shoppingStoreFilter)?.label} by your current preferred plan. Unpriced items remain disclosed in the plan summary.`}</p>${optimizerPanel}<div class="shopping-toolbar"><span>${icons.cart} <strong>${items.length}</strong> ingredients to pick up</span>${checked.length ? `<button class="button text" data-clear-checked>Clear checked items</button>` : ""}</div><div class="shopping-layout">${shopSection("sunday", "Sunday main shop", "Stock the week")}${shopSection("wednesday", "Wednesday fresh pickup", "Small and fresh")}</div>${extras.length ? `<div class="extras-note"><span>Extra recipe${extras.length > 1 ? "s" : ""} added manually: ${extras.map((id) => getRecipe(id).name).join(", ")}</span><button data-clear-extras>Remove extras</button></div>` : ""}</section>`;
}
...
document.addEventListener("click", async (event) => {
  ...
  if (shoppingMode) { shoppingOptimizationMode = shoppingMode.dataset.shoppingMode; userProfile = saveUserProfile({ ...userProfile, shopping: { ...userProfile.shopping, optimizationMode: shoppingOptimizationMode } }); store.set("fk-shopping-optimization-mode", shoppingOptimizationMode); render({ preserveScroll: true }); return; }
  ...
  if (saveStore) {
    ...
    const current = exactStoreFor(userProfile, retailerKey);
    const name = prompt(`Select exact ${retailer?.name || "store"} branch — enter branch name`, current?.name || "");
    if (!name) return;
    const id = prompt("Retailer store ID", current?.id || "");
    if (!id) return;
    const address = prompt("Store address", current?.address || "");
    const postalCode = (address.match(/\b\d{5}\b/) || [current?.postalCode || userProfile.postalCode])[0];
  ...
  const recipeReadinessButton = event.target.closest("[data-recipe-readiness]");
  if (recipeReadinessButton) { recipeReadinessFilter = recipeReadinessButton.dataset.recipeReadiness; render({ preserveScroll: true }); return; }
...
document.addEventListener("change", (event) => {
  if (event.target.matches("[data-meaningful-savings]")) { meaningfulSavings = Math.max(0, Number(event.target.value) || 0); userProfile = saveUserProfile({ ...userProfile, shopping: { ...userProfile.shopping, meaningfulSavings } }); store.set("fk-shopping-meaningful-savings", meaningfulSavings); render({ preserveScroll: true }); return; }
  if (event.target.matches("[data-max-shopping-trips]")) { const maxTrips = Math.min(5, Math.max(1, Number(event.target.value) || 1)); userProfile = saveUserProfile({ ...userProfile, shopping: { ...userProfile.shopping, maxTrips } }); render({ preserveScroll: true }); return; }
  if (event.target.matches("[data-shopping-radius]")) { const radiusMiles = Math.min(50, Math.max(1, Number(event.target.value) || 10)); userProfile = saveUserProfile({ ...userProfile, shopping: { ...userProfile.shopping, radiusMiles } }); render({ preserveScroll: true }); return; }
