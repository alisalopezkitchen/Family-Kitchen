...
function homeView() {
  ...
  const mealDays = week.days.filter((day) => day.recipeIds.length);
  return `<section class="home-hero">
    ...
    <aside class="tonight-card"><span class="card-label">Sunday table</span><p class="tiny-label">First up this week</p><h2>${getRecipe(mealDays[0].recipeIds[0]).name}</h2><p>with basmati rice, a crisp herb salad, and lemon-dill sauce</p><div class="tonight-meta"><span>${icons.clock} Sunday dinner</span><a href="#/week">View plan ${icons.arrow}</a></div></aside>
  </section>
  <section class="dashboard section-shell"><div class="section-title"><div><p class="eyebrow">At a glance</p><h2>Your week in the kitchen</h2></div><p>${mealDays.length} meals planned · ${week.days.length - mealDays.length} open evenings</p></div>
    <div class="dashboard-grid">
      <article class="panel plan-preview"><div class="panel-heading"><span class="icon-disc">${icons.leaf}</span><div><p class="tiny-label">Meals planned</p><h3>${mealDays.length} dinners, plenty of room</h3></div></div>${mealDays.map((day) => `<div class="mini-day"><strong>${day.day.slice(0, 3)}</strong><span>${escapeHtml(getRecipe(day.recipeIds[0]).name)}</span></div>`).join("")}<a class="panel-link" href="#/week">Full weekly plan ${icons.arrow}</a></article>
...
function weekView() {
  ...
  return `<section class="section-shell page">${pageHeader(`${week.label} · ${week.dateRange}`, "This week", "A flexible five-dinner rhythm with two intentional openings for life outside the kitchen.")}
    <div class="week-layout"><div class="day-list">${week.days.map((day) => `<article class="day-card ${day.recipeIds.length ? "" : "open-day"}"><div class="day-name"><span>${day.day.slice(0, 3)}</span><h2>${day.day}</h2></div><div class="day-content">${day.theme ? `<p class="tiny-label">${day.theme}</p>` : `<p class="tiny-label">Intentionally open</p>`}${day.recipeIds.map((id) => recipeLink(getRecipe(id))).join("")}${day.note ? `<p class="day-note">${day.note}</p>` : ""}</div></article>`).join("")}</div>
